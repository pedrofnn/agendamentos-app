...
const AgendamentoForm = () => {
  const Label = ({children, width = "w-full"}) => {
    return (
      <label className={`flex flex-col gap-1 mb-3 ${width}`}>{children}</label>
    )
  }
  const Input = ({type, inputName}) => {
    return (
      <input type={type} name={inputName} className={`p-1 border rounded border-grey-300 focus:border-blue-500 focus:ring-blue-500`}/>
    )
  }
  const Select = ({data}) => {
    return (
    ...
    <form className="w-96 mx-auto">
      ...
      <div className="flex justify-between">
        ...
        <Label width={"w-1/4 relative"}>Horário
          ...
          <span className="absolute top-1/2 right-2 z-10">clock</span>
        </Label>
      </div>
      <Label>Unidade
